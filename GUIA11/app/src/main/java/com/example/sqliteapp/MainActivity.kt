...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cmbCategorias = findViewById(R.id.cmbCategoria)
    ...
    override fun onClick(view: View) {
        ...
        if (db != null) {
            if (view === btnAgregar) {
                ...
                Toast.makeText(this, "No se puede conectar a la Base de datos", Toast.LENGTH_SHORT)
                    .show()
            }
        }
    }
    ...
    private fun vericarFormulario(opc: String): Boolean {
        ...
        if (opc === "insertar" || opc == "actualizar") {
            ...
            if (opc == "buscar") {
                if (idproducto.isEmpty()) {
                    idproducto_v = false
                    notificacion = "No se ha selecionado un producto"
                }
                response =
                    !(nombre_v == false || precio_v == false || cantidad_v == false || idproducto_v == false)
            } else {
